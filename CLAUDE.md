...
| `ContentGroupAdd.py` | REST API + mstrio_core | Add objects to a content group. Subcommands: `csv` (read GUIDs from a CSV file with a `GUID` column — additional columns ignored; resolves each GUID via `get_object_type_info()` for name/type), `folder` (read all non-hidden, non-folder contents from a folder GUID; resolves shortcuts to their target objects via `GET /objects/{id}?type=18`). Batches all objects into a single `PATCH /contentGroups/{id}/contents` call. Dry-run by default; `--apply` to execute. Content group specified by name or GUID. Writes results CSV with object GUID, name, type, source, and status. |
