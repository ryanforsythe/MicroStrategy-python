...
| `ProjectDuplicate.py` | mstrio-py + mstrio_core | Duplicate a project within or across environments (dev/qa/prod). Reads parameters from a YAML config file (`project_duplicate_config.yaml`). Supports same-environment (`DuplicationConfig`) and cross-environment (`CrossDuplicationConfig`) duplication with status polling. Dry-run by default; `--apply` to execute. |
